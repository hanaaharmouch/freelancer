...
public class ProjetsPorteurImp implements IProjetsPorteur {
	...
	public List<Projet> projetParMC(Integer mc) {
		...
		try { 
			PreparedStatement ps=conn.prepareStatement
					("select * from projet As pr,postulation As po where pr.id_postulation=po.id_postulation and pr.id_porteur=?");
			ps.setInt(1, mc);
